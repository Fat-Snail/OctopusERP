...
<template>
  <Teleport to="body">
    <template v-if="ctx.open.value">
      ...
      <div
        :class="cn(
          'fixed left-1/2 top-[10%] z-50 -translate-x-1/2 grid w-full max-w-lg gap-4 border border-border bg-background p-6 shadow-lg sm:rounded-lg max-h-[80vh] overflow-y-auto',
          props.class,
        )"
      >
        <slot />
