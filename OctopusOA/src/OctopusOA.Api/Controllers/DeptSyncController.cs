...
/// <summary>部门同步接口（由 UMC 调用，HMAC 签名验证）</summary>
[ApiController]
[AllowAnonymous]
public class DeptSyncController : ControllerBase
{
    private readonly ContactService _service;
    private readonly IConfiguration _config;

    public DeptSyncController(ContactService service, IConfiguration config)
    {
        _service = service;
        _config = config;
    }

    [HttpPost("/api/contact/dept/sync")]
    public async Task<IActionResult> Sync()
    {
        ...
        var payload = JsonSerializer.Deserialize<DeptSyncPayload>(body,
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true });

        if (payload == null)
            return BadRequest(new { code = 400, msg = "无效的请求体" });

        _service.SyncDept(payload);
