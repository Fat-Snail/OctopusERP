...
async function scrollActive() {
  await nextTick()
  const el = document.querySelector<HTMLElement>('.erp-tab.active')
  el?.scrollIntoView({ inline: 'nearest', block: 'nearest', behavior: 'smooth' })
}
defineExpose({ scrollActive })
</script>
...
<template>
  ...
  >
    <div
      ...
      :key="tab.path"
      :class="[
        'erp-tab relative inline-flex items-center gap-1.5 h-[30px] px-3 text-[12px] shrink-0',
        ...
        tab.path === tabsStore.activeTab
          ? 'bg-card border-border text-foreground font-medium active'
    ...
    >
      ...
      <span
        ...
        class="absolute bottom-0 left-0 right-0 h-[2px] bg-primary rounded-t"
        style="bottom:-1px;"
      />
  ...
  <!-- Context Menu (teleport to body so it's never clipped) -->
  <Teleport to="body">
    <div
      v-if="ctxVisible"
