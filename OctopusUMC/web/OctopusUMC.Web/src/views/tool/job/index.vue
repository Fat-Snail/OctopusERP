...
import type { JobResponse } from '@/api/system/types'
...
async function handleRun(row: JobResponse) {
  ...
  await put('/monitor/job/run', { jobId: row.jobId })
}

async function toggleStatus(row: JobResponse) {
  const newStatus = row.status === 1 ? 0 : 1
  await put('/monitor/job/status', { jobId: row.jobId, status: newStatus })
  loadData()
}
          ...
          <tr v-else v-for="(row, idx) in list" :key="row.jobId" class="border-b border-border last:border-0 hover:bg-subtle">
            ...
            <td class="px-4 py-2 text-right">
              <div class="flex items-center justify-end gap-1">
                ...
                <button class="h-6 px-2 text-[11px] text-warning hover:bg-warning/10 rounded cursor-pointer border-0 bg-transparent flex items-center gap-1" @click="toggleStatus(row)">
                  <Pause v-if="row.status === 1" :size="10" /><RotateCcw v-else :size="10" /> {{ row.status === 1 ? '暂停' : '恢复' }}
                </button>
    ...
    <DialogContent class="w-[560px]">
      ...
      <div class="grid grid-cols-2 gap-3 py-2">
        ...
        <div class="col-span-2">
          ...
          <input v-model="formData.invokeTarget" placeholder="如：ryTask.ryParams('ry')" class="w-full h-8 px-2.5 rounded-[var(--radius)] border border-input bg-background text-[12px] font-mono-tnum focus:outline-none focus:ring-1 focus:ring-ring" />
        </div>
        <div>
          <label class="block text-[12px] font-medium text-foreground mb-1">Cron表达式 <span class="text-danger">*</span></label>
          <input v-model="formData.cronExpression" placeholder="0 0 * * * ?" class="w-full h-8 px-2.5 rounded-[var(--radius)] border border-input bg-background text-[12px] font-mono-tnum focus:outline-none focus:ring-1 focus:ring-ring" />
        </div>
        <div>
