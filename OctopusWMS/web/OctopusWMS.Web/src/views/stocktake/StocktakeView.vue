...
import { getStocktakeList } from '@/api/stocktake'
import type { StocktakeListRow, StocktakeStatus } from '@/api/stocktake/types'
...
async function loadData() {
  loading.value = true
  try {
    const result = await getStocktakeList({ pageNum: 1, pageSize: 50 })
...
function statusBadgeClass(status: StocktakeStatus): string {
  ...
  if (status === 'in_progress') return 'bg-primary-soft text-[var(--primary-soft-fg)]'
  if (status === 'pending') return 'bg-[color-mix(in_oklch,var(--warning)_18%,transparent)] text-[var(--warning)]'
  return 'bg-muted text-muted-foreground'
}

const statusLabel: Record<StocktakeStatus, string> = {
  pending: '待盘点',
  in_progress: '盘点中',
  completed: '已完成',
  cancelled: '已取消',
...
const columnHelper = createColumnHelper<StocktakeListRow>()
const columns = [
  columnHelper.accessor('stocktakeCode', {
    header: '盘点单号',
...
<template>
  <div>
    <PageHeader title="盘点管理" :sub="`共 ${total} 条盘点单`">
      <template #actions>
        <button
